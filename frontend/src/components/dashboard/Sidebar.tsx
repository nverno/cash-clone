...
const Sidebar: FC<SidebarProps> = props => {
  ...
  const [activeLink, setActiveLink] = React.useState<SidebarLinkType>('activity');

  React.useEffect(() => {
    const parts = location.pathname.split('/');
    const current = parts[parts.length - 1];
    if (current !== activeLink)
      setActiveLink(current as SidebarLinkType)
  }, [location]);
  ...
  return (
    <div className="!h-full account-navigation-bar flex-container">
      ...
      <nav className="nav-items">
        <SidebarLink type='activity' title='Activity'>
          <svg viewBox="0 0 100 100" className="inline-svg activity-badge">
            <g>
              ...
              <text x="50" y="70" fontSize="50" textAnchor="middle">1</text>
            </g>
          </svg>
        ...
        <a href="#" data-link-label="Navbar Create Payment" title="New" className="nav-item create-payment">
